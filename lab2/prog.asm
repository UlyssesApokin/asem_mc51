...
ecw		equ p0.0	;движение перекл-я по ч.с. (engine clockwise)
eccw	equ p0.1	;движение перкл-я против ч.с. (engine counterclockwise)
conv	equ p0.2	;движение конвейера с заготовками
std		equ p3		;вывод МК для сообщения об ошибках и выводе информации
...
;==============================================================================
		...
		lcall ERR			;уведомление об ошибке и завершение работы
;==============================================================================
...
NSEQ:	jb d1, $		;если заготовки нет, то ожидать появления заготовки,
		...
		lcall WAIT		;задержка в 1с (чтобы два раза не посчитать одну заг.)
...
;==============================================================================
;	ПРОЦЕДУРА ОЖИДАНИЯ (ЗАДЕРЖКА) (980 мс)
;==============================================================================
WAIT:	mov ACC, #25
M1:		mov R3, #18
M2:		djnz R3, M2
...
;==============================================================================
;	ПРОЦЕДУРА ПРЕДУПРЕЖДЕНИЯ ОПЕРАТОРА О СБОЕ/НЕИСПРАВНОСТИ
;==============================================================================
;------------------------------------------------------------------------------
;	если перекл-ь не оказался в положениях 1, 2 или 3, то необходимо
;	сообщить об ошибке оператору конвейера.
;	для этого отправляем на порт, предназначенный для вывода информации
;	число 119.
;	это будет кодом ошибки: ПОЛОЖЕНИЕ ПЕРЕКЛ-Я КОНВЕЙЕРА НЕ ОПРЕДЕЛЕНО
;	выбор конкретно этого числа ничем не обоснован и по-просту случаен
;------------------------------------------------------------------------------
ERR:	mov std, #119		;вывод кода ошибки в порт вывода инф-ии
		ljmp FIN			;аварийное завершение программы
		ret
