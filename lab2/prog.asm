...
std		equ p2		;вывод МК для сообщения об ошибках и выводе информации
					;ИСП-ТЬ ТОЛЬКО ПИНЫ 0 -- 3 !
;------------------------------------------------------------------------------
d1		equ p2.4	;датчик d1 (кол-во заготовок)
d2		equ p2.5	;датчик d2 (первое положение)
d3		equ p2.6	;датчик d3 (второе положение)
d4		equ p2.7	;датчик d4 (третье положение)
...
;==============================================================================
BEGIN:	setb stop			;отжатие кнопки стоп
		setb conv			;выключение конвейера
		jb up, $			;если кнопка СТАРТ НЕ нажата, то ожидание нажатия,
							;иначе переход к следующей команде
		setb up				;выключение кнопки
...
;	перемещение деталей
;==============================================================================
	cjne c_src, #N_src, NSEQ	;если кол-во прошедших заготовок не равно
...
NSEQ:	jb d1, $		;если заготовки нет, то ожидать появления заготовки,
		...
		lcall STDOUT	;выводим кол-во заготовок на экран
...
;------------------------------------------------------------------------------
		...
		jb ACC.0, TT1		;(проверка первого бита)
		sjmp TT2			;(если первый бит не совпал, то второй можно не
							;проверять, переход к след. случаю)
TT1:	jb ACC.1, CASE3		;(проверка второго бита)
;------------------------------------------------------------------------------
TT2:	jb ACC.0, CASE1		;если в первом положении  (0000 0001)
							;перейти по метке, иначе след. команда
;------------------------------------------------------------------------------
		jb ACC.1, CASE2		;если в втором положении  (0000 0010)
							;перейти по метке, иначе след. команда
		lcall ERR			;перем-м перекл-ь принудительно в 1-е положение
;------------------------------------------------------------------------------
...
MVCW1:	clr ecw			;включить вращение по часовой
		lcall SAFE		;вызов функции проверки нажатия кнопки СТОП
...
SAFE:	jb stop, S_EX	;если кнопка не нажата, завершить подпрограмму, иначе
						;след. команда
		setb stop		;отжатие кнопки стоп
		jb up, $		;ожидать нажатие кнопки СТАРТ, иначе след. ком.
		setb up			;отжатие кнопки
		mov c_src, #0	;обнулить счетчик заготовок
S_EX:	ret


;==============================================================================
;	ПРОЦЕДУРА ВЫВОДА ЧИСЕЛ НА ЭКРАН
;==============================================================================
STDOUT:	mov ACC, c_src	;заносим кол-во деталей в акк. чтобы с ними работать
		anl A, #0Fh		;заноляем верхние незначащие биты
		mov c_src, ACC	;возвращаем кол-во дет. назад в свой регистр
		mov ACC, std	;заносим значение портов в акк. чтобы с ними работать
		anl A, #0F0h	;заноляем нижние незначащие биты
		orl A, c_src	;заносим кол-во деталей в этот регистр
		mov std, ACC	;передаем эту информацию в порт вывода
		ret
